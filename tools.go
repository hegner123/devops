...
func toolDefinitions() []map[string]any {
	return []map[string]any{
		...
		},
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"key_path":         prop("string", "SSH private key path (default: ~/.ssh/id_ed25519, fallback: ~/.ssh/id_rsa)"),
				},
		...
		{
			"name":        "devops_filter_add",
			"description": "Add a custom command filter for a host. Commands matching the pattern will be blocked on the agent.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
					"host":     prop("string", "Server hostname or IP"),
					"pattern":  prop("string", "Substring pattern to block (case-insensitive match)"),
					"category": propDefault("string", "Filter category", "custom"),
					"reason":   prop("string", "Human-readable reason for the filter"),
				},
				"required": []string{"host", "pattern"},
			},
		},
		{
			"name":        "devops_filter_list",
			"description": "List custom command filters. Optionally filter by host.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
					"host": prop("string", "Filter by host"),
				},
			},
		},
		{
			"name":        "devops_filter_remove",
			"description": "Remove a custom command filter for a host.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
					"host":    prop("string", "Server hostname or IP"),
					"pattern": prop("string", "Pattern to remove"),
				},
				"required": []string{"host", "pattern"},
			},
		},
		{
			"name":        "devops_filter_sync",
			"description": "Sync command filters to the remote agent config file. Writes filters from the local DB plus port/reboot/shutdown settings to /etc/devops-agent/filters.json on the host.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
					"host":           prop("string", "Server hostname or IP"),
